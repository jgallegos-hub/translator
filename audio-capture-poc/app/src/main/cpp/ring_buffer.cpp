...
std::size_t RingBuffer::write(const int16_t* src, std::size_t count) {
    ...
    // If the incoming chunk is larger than the buffer, keep only the tail.
    ...
    // If we would overflow, advance the read position so we drop the oldest.
    ...
    // Copy with wrap-around. write_pos grows monotonically; index = write_pos % capacity_.
